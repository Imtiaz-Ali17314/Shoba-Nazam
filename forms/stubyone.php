...
if($fdate == "" && $ldate == "")
{
    ...
    FROM student s left join `benazmi` b on s.id=b.sid where s.sname like '%$name%' and 
    s.fathername like '%$fathername%' and s.code like '%$code%' and s.class like '%$class%' 
...
else if($fdate == "")
{
    ...
    max(date) as endDate, COUNT(b.sid) as total  
    FROM student s left join `benazmi` b on s.id=b.sid where s.sname like '%$name%' and 
    s.fathername like '%$fathername%' and s.code like '%$code%' and s.class like '%$class%' and b.date <= '$ldate'
    group by b.sid order by total desc" ;
...
else if($ldate == "")
{
    ...
    max(date) as endDate, COUNT(b.sid) as total  
    FROM student s left join `benazmi` b on s.id=b.sid where s.sname like '%$name%' and 
    s.fathername like '%$fathername%' and s.code like '%$code%' and s.class like '%$class%' and b.date  >= '$fdate'
    group by b.sid order by total desc" ;
...
else 
{
    ...
    max(date) as endDate, COUNT(b.sid) as total  
    FROM student s left join `benazmi` b on s.id=b.sid where s.sname like '%$name%' and 
    s.fathername like '%$fathername%' and s.code like '%$code%' and s.class like '%$class%' and b.date between '$fdate' and '$ldate' 
    group by b.sid order by total desc" ;
...
<html dir="rtl">
                    ...
                    <form method="get" action="stubyone.php" id="form1">
                        ...
                        <thead>
                            ...
                            <tr id="searchbox">
                                        ...
                                        <td style="text-align: right;">
                                            <input type="date"  name="fdate" id="fdate" placeholder="شروع کی تاریخ" style="outline: none;width: 165px;"  value="<?php echo $fdate ?>">
                                        </td>
                                        <td>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><a href="detailbyone.php?sid=<?php echo $row['sid'] ?>&&fdate=<?php echo $fdate ?>&&ldate=<?php echo $ldate ?>"><button type="button">تفصیل</button></a></td>
                        ...
                        <tfoot>
                            <tr>
                                <td>  
                                    <input type="date" name="ldate" id="ldate" placeholder="آخری تاریخ" style="outline: none;" value="<?php echo $ldate ?>">
                                </td>
                                <td></td>
                                <td></td>
                                <td></td>
    ...
    <script src="../java/stubyone.js"> </script>
